...
# }`,
    ...
    quiz: [
      ...
      { type: "mcq", question: "Quel code de statut HTTP reçois-tu quand une ressource est créée avec succès via un POST ?", options: JSON.stringify(["200 OK", "201 Created", "400 Bad Request", "404 Not Found"]), correctAnswer: "201 Created", explanation: "Le code 201 Created confirme qu'une nouvelle ressource a bien été créée suite à une requête POST. Le code 200 OK indique un succès générique (souvent pour GET)." },
    ],
  },
...
stripe trigger checkout.session.completed`,
    ...
    quiz: [
      ...
      { type: "mcq", question: "Quel événement webhook Stripe déclenche la confirmation d'un paiement réussi via Checkout ?", options: JSON.stringify(["payment_intent.created", "checkout.session.completed", "customer.subscription.created", "invoice.payment_failed"]), correctAnswer: "checkout.session.completed", explanation: "L'événement checkout.session.completed est envoyé par Stripe quand un client termine avec succès le processus de paiement Checkout. C'est le signal pour débloquer l'accès ou livrer le produit." },
    ],
  },
...
export function verifyWebhookSignature({
  ...
  return crypto.timingSafeEqual(
    ...
    quiz: [
      ...
      { type: "mcq", question: "Quel outil permet d'inspecter visuellement les requêtes webhook reçues en local via ngrok ?", options: JSON.stringify(["Le dashboard Stripe à l'adresse dashboard.stripe.com", "L'interface ngrok à l'adresse localhost:4040", "La console du navigateur (DevTools)", "Le fichier .env du projet"]), correctAnswer: "L'interface ngrok à l'adresse localhost:4040", explanation: "ngrok fournit une interface web sur localhost:4040 qui affiche toutes les requêtes reçues avec leurs headers, payloads et codes de réponse. C'est un outil précieux pour déboguer les webhooks." },
    ],
  },
...
export async function callClaude(userMessage: string): Promise<string> {
  ...
  return textBlock?.type === 'text' ? textBlock.text : '';
    ...
    quiz: [
      ...
      { type: "mcq", question: "Quelle est la bonne pratique pour stocker sa clé API OpenAI ou Anthropic dans un projet Next.js ?", options: JSON.stringify(["Dans un fichier config.js à la racine du projet", "Dans une variable d'environnement (.env) côté serveur uniquement", "Dans le localStorage du navigateur", "Dans un commentaire en haut du fichier API"]), correctAnswer: "Dans une variable d'environnement (.env) côté serveur uniquement", explanation: "Les clés API doivent être dans un fichier .env (ajouté au .gitignore) et utilisées uniquement côté serveur. Une variable préfixée NEXT_PUBLIC_ serait exposée au client, ce qui est dangereux pour une clé secrète." },
    ],
  },
...
// Si seulement Slack échoue, on répond quand même 200`,
    ...
    quiz: [
      ...
      { type: "mcq", question: "Pourquoi ne faut-il jamais afficher le message d'erreur brut du serveur à l'utilisateur en cas d'échec ?", options: JSON.stringify(["Parce que les messages d'erreur sont trop longs", "Parce qu'ils peuvent révéler des informations sensibles sur l'infrastructure", "Parce que l'utilisateur ne comprend pas l'anglais", "Parce que cela ralentit le chargement de la page"]), correctAnswer: "Parce qu'ils peuvent révéler des informations sensibles sur l'infrastructure", explanation: "Les messages d'erreur internes peuvent contenir des chemins de fichiers, des noms de variables d'environnement, ou des détails sur la stack technique. Il faut toujours renvoyer un message générique à l'utilisateur et logger l'erreur détaillée côté serveur." },
    ],
  },
];
