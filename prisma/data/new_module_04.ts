...
-- C'est ce qu'on appelle une relation "One-to-Many" (1 user → plusieurs orders)` },
    ...
    quiz: [
      ...
      { type: "mcq", question: "Quel type SQL utilise-t-on pour stocker un identifiant unique universel ?", options: JSON.stringify(["TEXT", "INTEGER", "UUID", "BOOLEAN"]), correctAnswer: "UUID", explanation: "Le type UUID (Universal Unique Identifier) génère un identifiant unique comme 'a1b2c3d4-e5f6-...'. C'est le type recommandé pour les clés primaires car il est unique et non prédictible." },
    ],
  },
  {
    order: 2,
...
# const supabaseKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY` },
    ...
    quiz: [
      ...
      { type: "mcq", question: "Où stocke-t-on les clés API Supabase dans un projet Next.js ?", options: JSON.stringify(["Directement dans le code source", "Dans un fichier .env.local (variables d'environnement)", "Dans un commentaire HTML", "Dans le fichier package.json"]), correctAnswer: "Dans un fichier .env.local (variables d'environnement)", explanation: "Les clés API se stockent dans un fichier .env.local qui ne doit JAMAIS être commité dans Git. On y accède via process.env.NEXT_PUBLIC_SUPABASE_URL et process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY." },
    ],
  },
  {
    order: 3,
...
Génère le SQL de migration PostgreSQL."` },
    ...
    quiz: [
      ...
      { type: "mcq", question: "Quelle est la meilleure façon d'utiliser Claude pour le SQL ?", options: JSON.stringify(["Lui demander de mémoriser toute la syntaxe SQL", "Lui donner le schéma de ta base et décrire ce que tu veux en français", "Ne jamais utiliser Claude pour le SQL", "Copier des requêtes depuis Stack Overflow"]), correctAnswer: "Lui donner le schéma de ta base et décrire ce que tu veux en français", explanation: "Le game-changer : donne à Claude le schéma de ta base (CREATE TABLE) et décris en français ce que tu veux. Il génère la requête exacte. Tu n'as plus à lutter avec la syntaxe." },
    ],
  },
  {
    order: 4,
...
async function updatePassword(newPassword) {
  ...
  alert('Mot de passe mis à jour !')
    ...
    quiz: [
      ...
      { type: "mcq", question: "Combien d'étapes faut-il pour configurer l'OAuth Google dans Supabase ?", options: JSON.stringify(["1 étape", "3 étapes (activer dans Supabase, créer credentials Google, coller les clés)", "5 étapes", "10 étapes"]), correctAnswer: "3 étapes (activer dans Supabase, créer credentials Google, coller les clés)", explanation: "La configuration Google OAuth se fait en 3 étapes : 1) Activer Google dans Supabase Dashboard, 2) Créer les credentials sur Google Cloud Console, 3) Coller le Client ID et Secret dans Supabase. Ensuite, une seule ligne de code suffit." },
    ],
  },
  {
    order: 5,
