...
export const MODULE_7_LESSONS = [
  // ─── LEÇON 1 ───
  {
    ...
    quiz: [
      ...
      { type: "mcq", question: "Comment justifier le prix d'un chatbot à 2 000€ face à un prospect ?", options: JSON.stringify(["Expliquer le temps de développement nécessaire", "Calculer les économies générées (temps SAV, leads qualifiés) et montrer un ROI rapide", "Comparer avec les prix des concurrents", "Proposer un paiement en plusieurs fois"]), correctAnswer: "Calculer les économies générées (temps SAV, leads qualifiés) et montrer un ROI rapide", explanation: "Le client ne paie pas pour tes heures — il paie pour le résultat. Un chatbot qui économise 5h/semaine au client, c'est environ 200€/semaine. Le ROI est atteint en 10 semaines. C'est l'argument le plus convaincant." },
    ],
  },
  ...
  // ─── LEÇON 2 ───
  {
    ...
    quiz: [
      ...
      { type: "mcq", question: "Quel est le principal inconvénient de Voiceflow sur les plans bas de gamme ?", options: JSON.stringify(["Il ne supporte pas l'IA", "Le branding Voiceflow reste visible sur le widget", "Il est impossible d'ajouter une base de connaissances", "Il ne fonctionne que sur mobile"]), correctAnswer: "Le branding Voiceflow reste visible sur le widget", explanation: "Sur les plans basiques, le logo Voiceflow apparaît sur le widget chat. Pour un rendu 100% professionnel sans branding tiers, il faut soit un plan supérieur, soit passer en solution custom ou Botpress avec white-label." },
    ],
  },
...
<script type="text/javascript">
  (function(d, t) {
    ...
    quiz: [
      ...
      { type: "mcq", question: "Quel est le flow conversationnel de base d'un chatbot Voiceflow ?", options: JSON.stringify(["API → Réponse → Fin", "Start → Message d'accueil → Capture input → Knowledge Base Query → Réponse IA → Boucle ou fin", "Webhook → Traitement → Email", "Login → Dashboard → Actions"]), correctAnswer: "Start → Message d'accueil → Capture input → Knowledge Base Query → Réponse IA → Boucle ou fin", explanation: "Le flow standard commence par un bloc Start, suivi d'un message d'accueil (Speak), la capture de la question utilisateur, l'interrogation de la Knowledge Base, l'affichage de la réponse IA, puis une condition pour savoir si l'utilisateur a d'autres questions." },
    ],
  },
...
export default function Home() {
  return (
    ...
    quiz: [
      ...
      { type: "mcq", question: "Comment le client (navigateur) lit-il le flux de streaming renvoyé par l'API route ?", options: JSON.stringify(["Avec un appel fetch() classique et response.json()", "Avec response.body.getReader() qui lit le stream chunk par chunk", "Avec un setInterval qui poll l'API toutes les secondes", "Avec un EventSource SSE natif"]), correctAnswer: "Avec response.body.getReader() qui lit le stream chunk par chunk", explanation: "Le ReadableStream est lu via response.body.getReader() dans une boucle while. Chaque chunk est décodé avec TextDecoder et ajouté au message en cours. C'est ce qui permet l'affichage progressif des tokens à l'écran." },
    ],
  },
...
\${context}\`
  ...
  return new Response(readableStream, {
    ...
    quiz: [
      ...
      { type: "mcq", question: "Quelle base de données vectorielle est recommandée pour commencer gratuitement avec pgvector ?", options: JSON.stringify(["MongoDB Atlas", "Supabase avec l'extension pgvector", "MySQL avec FULLTEXT", "Redis"]), correctAnswer: "Supabase avec l'extension pgvector", explanation: "Supabase offre pgvector gratuitement jusqu'à 500 MB, ce qui est largement suffisant pour débuter. L'extension pgvector s'active avec 'create extension vector' et permet de stocker des vecteurs et faire des recherches par similarité cosinus directement en SQL." },
    ],
  },
...
\`;` },
    ...
    quiz: [
      ...
      { type: "mcq", question: "Quel est l'élément clé du system prompt pour éviter que le chatbot invente des informations ?", options: JSON.stringify(["Limiter le nombre de tokens à 100", "Inclure une instruction explicite de ne jamais inventer et de rediriger vers le support si la réponse n'est pas dans le contexte", "Utiliser une température de 1.0", "Ajouter un disclaimer légal en début de conversation"]), correctAnswer: "Inclure une instruction explicite de ne jamais inventer et de rediriger vers le support si la réponse n'est pas dans le contexte", explanation: "Le system prompt doit contenir une section 'NE JAMAIS' et une instruction de fallback claire : 'Si la réponse n'est pas dans le contexte, réponds : Je n'ai pas cette information, contactez support@...' Cela empêche les hallucinations et maintient la confiance du client." },
    ],
  },
];
