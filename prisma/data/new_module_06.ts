...
export const MODULE_6_LESSONS = [
  // ─── LEÇON 1 ───
  {
    ...
    quiz: [
      ...
      { type: "mcq", question: "Parmi ces workflows, lequel est considéré comme le plus facile à livrer pour un débutant ?", options: JSON.stringify(["Qualification de leads par IA", "Veille concurrentielle + digest", "Leads email → CRM auto", "Onboarding client automatisé"]), correctAnswer: "Leads email → CRM auto", explanation: "Le workflow 'Leads email → CRM auto' est classé comme facile, avec un prix de vente de 600-1 200€. C'est le type de workflow idéal pour commencer, livrer vite et générer ses premiers avis positifs." },
    ],
  },
  ...
  "Statut": "Nouveau"
    ...
    quiz: [
      ...
      { type: "true_false", question: "Dans Make, il est inutile de gérer les erreurs car les scénarios fonctionnent toujours correctement.", options: JSON.stringify(["Vrai", "Faux"]), correctAnswer: "Faux", explanation: "La gestion des erreurs est essentielle pour un scénario professionnel. Make permet de définir le comportement en cas d'échec : arrêt, retry automatique, ou notification. Un scénario sans gestion d'erreurs va planter sans prévenir." },
      { type: "mcq", question: "Quel est le premier module à ajouter dans un nouveau scénario Make ?", options: JSON.stringify(["Un module d'action (Gmail, Slack, etc.)", "Un module déclencheur (Watch, Webhook, Schedule)", "Un module de filtre", "Un module de transformation JSON"]), correctAnswer: "Un module déclencheur (Watch, Webhook, Schedule)", explanation: "Le déclencheur est toujours le point de départ d'un scénario. C'est lui qui démarre l'exécution : surveillance d'un Google Sheet (Watch Rows), réception d'un webhook, ou déclenchement programmé (Schedule)." },
    ],
  },
...
# Accès : http://localhost:5678` },
  ...
  "body": {
    ...
    quiz: [
      ...
      { type: "mcq", question: "Quelle méthode de déploiement est recommandée pour n8n en production chez un client ?", options: JSON.stringify(["npx n8n sur le PC du client", "Docker sur un VPS avec reverse proxy Nginx + SSL", "Hébergement sur un site WordPress", "Installation sur un smartphone"]), correctAnswer: "Docker sur un VPS avec reverse proxy Nginx + SSL", explanation: "Pour la production, Docker assure la stabilité et la portabilité. Un VPS à 5-10€/mois (OVH, Hetzner) avec Nginx en reverse proxy et un certificat SSL fournit une infrastructure professionnelle, sécurisée et facilement maintenable." },
    ],
  },
  ...
  // ─── LEÇON 4 ───
  {
    ...
    quiz: [
      ...
      { type: "true_false", question: "Zapier est toujours le meilleur choix pour les e-commerces car il a le plus grand nombre d'intégrations.", options: JSON.stringify(["Vrai", "Faux"]), correctAnswer: "Faux", explanation: "Malgré ses 6 000+ intégrations, Zapier peut devenir très coûteux à fort volume. Un e-commerce avec 200 commandes/jour et 5 zaps par commande = 30 000 tâches/mois = 400€+/mois chez Zapier, alors que Make ferait le même travail pour 29€/mois." },
      ...
      { type: "mcq", question: "Pourquoi Zapier peut devenir un piège de coûts pour un client à fort volume ?", options: JSON.stringify(["Parce que Zapier facture par nombre d'intégrations connectées", "Parce que chaque tâche (action dans un Zap) est comptée et le prix augmente vite avec le volume", "Parce que Zapier facture le stockage des données", "Parce que Zapier n'offre pas de support technique"]), correctAnswer: "Parce que chaque tâche (action dans un Zap) est comptée et le prix augmente vite avec le volume", explanation: "Zapier facture par tâche (chaque action dans un Zap). 1 000 tâches semblent beaucoup, mais un e-commerce actif peut les consommer en quelques jours. À 30 000 tâches/mois, la facture dépasse 400€/mois." },
    ],
  },
  ...
  "usage": { "input_tokens": 312, "output_tokens": 47 }
    ...
    quiz: [
      ...
      { type: "mcq", question: "Dans le workflow de qualification de leads, que fait le router Make après l'analyse IA ?", options: JSON.stringify(["Il envoie tous les leads au même commercial", "Il route les leads vers différents chemins selon leur score IA (senior, junior, nurturing)", "Il supprime les leads avec un mauvais score", "Il renvoie les leads vers Claude pour une deuxième analyse"]), correctAnswer: "Il route les leads vers différents chemins selon leur score IA (senior, junior, nurturing)", explanation: "Le router Make dirige les leads selon leur score : 8-10 vers un commercial senior, 4-7 vers un commercial junior, 1-3 vers une séquence email nurturing automatique. Le commercial reçoit un briefing IA et arrive au call déjà préparé." },
    ],
  },
...
IMPORTANT : Retourne UNIQUEMENT le code HTML complet, sans backticks ni commentaires.` },
  ...
  "claude_analysis": {
    ...
    quiz: [
      ...
      { type: "mcq", question: "Quel score de pertinence minimum est recommandé pour inclure un article dans le digest final ?", options: JSON.stringify(["Score ≥ 3", "Score ≥ 5", "Score ≥ 7", "Score ≥ 9"]), correctAnswer: "Score ≥ 7", explanation: "Le filtre recommandé est score ≥ 7 sur 10. Cela réduit le nombre d'articles à 5-15 éléments pertinents par digest, ce qui est optimal pour la lisibilité. Un seuil trop bas noierait le client dans l'information, un seuil trop haut risquerait de manquer des articles importants." },
    ],
  },
];
