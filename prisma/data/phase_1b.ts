...
export const LESSONS = [
  ...
  // ═══════════════════════════════════════════════════
  {
    ...
    slug: "securite-agent-ia-pourquoi",
    duration: "45 min",
...
<p>Il y a une confusion très courante chez les débutants : ils pensent qu'un agent IA comme OpenClaw, c'est "juste un ChatGPT amélioré". C'est une erreur fondamentale, et cette erreur peut te coûter très cher.</p>
...
<p>Imagine concrètement : ton agent envoie des emails à tes clients, déploie du code en production sur tes serveurs, gère ton calendrier, traite des paiements via Stripe, et accède à tes repos GitHub. Si quelqu'un prend le contrôle de cet agent — ou même simplement accède aux données qu'il manipule — les conséquences sont catastrophiques. On parle de pertes financières directes, de violations de données personnelles, de destruction de code source, et de dommages réputationnels potentiellement irréversibles.</p>
...
<ol>
<li>Liste toutes les API keys que ton agent OpenClaw utilise actuellement (ou utilisera)</li>
...
<p><strong>Livrable :</strong> Un document avec ton audit de sécurité initial. Tu le mettras à jour au fur et à mesure du module.</p>`,
  },
  {
    ...
    slug: "hardware-mac-mini-serveur-dedie",
    duration: "50 min",
...
<p>Avant de parler de VPN, de firewall ou de configuration logicielle, il faut commencer par la base : la machine physique sur laquelle ton agent va tourner. C'est la fondation de tout le reste — et comme pour une maison, si la fondation est bancale, tout ce que tu construis dessus est fragile.</p>
...
<p>Voici pourquoi :</p>
<ul>
<li><strong>Silencieux</strong> : Zéro bruit en utilisation normale. Le ventilateur ne se déclenche que sous charge intense, et même là, il est à peine audible. Tu peux poser ton Mac Mini dans ton salon, ta chambre, ou ton bureau sans aucune nuisance sonore. Compare ça à un serveur Dell qui fait le bruit d'un aspirateur, ou même à un mini PC Windows dont le petit ventilateur siffle en permanence.</li>
<li><strong>Compact</strong> : 12,7 cm de côté pour le M2, encore plus petit pour le M4. Il se glisse derrière un écran, sur une étagère, dans un tiroir. Tu n'as pas besoin d'un rack serveur ou d'une pièce dédiée. Certains utilisateurs le fixent même derrière leur TV avec un support VESA.</li>
<li><strong>Puissant</strong> : La puce Apple Silicon (M2 ou M4) est une merveille d'efficacité. Elle gère facilement OpenClaw + plusieurs agents en parallèle + Docker + des services de monitoring, le tout sans broncher. Le Neural Engine intégré est un bonus pour les tâches de machine learning local. Tu auras de la marge de manœuvre pour scaler tes opérations.</li>
...
<ul>
<li><strong>Mac Mini M2 (ou M4)</strong> : Le modèle de base suffit amplement pour 1 à 3 agents tournant en parallèle. Le M4 est plus récent et plus efficace énergétiquement, mais le M2 fait parfaitement le travail. Si tu trouves un M2 reconditionné en promotion, c'est une excellente affaire.</li>
...
<li><strong>Connexion : Ethernet obligatoire</strong> : Toujours, toujours, toujours préférer le câble Ethernet au Wi-Fi pour un serveur. Le Wi-Fi est pratique pour un laptop, mais pour une machine qui doit être connectée 24/7, c'est un risque inutile : déconnexions aléatoires, latence variable, interférences. Un câble Ethernet à 5€, c'est une connexion stable, rapide et fiable. Si ton Mac Mini est loin de ton routeur, investis dans un câble Ethernet long ou un adaptateur CPL (courant porteur en ligne) — c'est mieux qu'un Wi-Fi instable.</li>
</ul>
...
<li><strong>Ancien Mac que tu n'utilises plus</strong> : Si tu as un vieux MacBook ou iMac qui traîne, c'est une option gratuite pour démarrer. Tant qu'il supporte macOS Ventura ou plus récent et qu'il a 8 Go de RAM, ça peut faire l'affaire temporairement. Inconvénient : consommation électrique plus élevée et les laptops ne sont pas conçus pour tourner 24/7 (problèmes de batterie et de chaleur).</li>
</ul>
...
<li><strong>Docker</strong> : Installe Docker Desktop (plus simple, interface graphique) ou Colima (plus léger, en ligne de commande). Colima est recommandé pour un serveur car il consomme moins de ressources. Installation via Homebrew : <code>brew install colima docker</code> puis <code>colima start</code>. Docker te permettra de conteneuriser tes services pour une meilleure isolation et gestion.</li>
<li><strong>Node.js</strong> : Installe Node.js via nvm (Node Version Manager) plutôt que directement via Homebrew. nvm te permet de gérer plusieurs versions de Node facilement. Installation : <code>brew install nvm</code>, puis <code>nvm install --lts</code> pour la dernière version stable. OpenClaw nécessite Node.js 18 ou supérieur.</li>
</ol>
...
<p>Une fois configuré, ton Mac Mini n'a besoin ni d'écran, ni de clavier, ni de souris. Tu le contrôles entièrement via SSH depuis ton PC principal, ton laptop, ou même ton téléphone avec une app comme Termius. C'est le mode "headless" — exactement comme un vrai serveur dans un datacenter.</p>
<p>En pratique, ton Mac Mini sera branché sur deux câbles : l'alimentation et l'Ethernet. C'est tout. Tu peux le ranger sur une étagère, derrière un meuble, ou même le fixer au mur avec un support adapté. Il fait son travail en silence, 24/7, sans que tu aies besoin de le toucher physiquement.</p>
...
<li>Installe Homebrew (Mac) ou les paquets essentiels (Linux) : git, curl, node, docker</li>
...
<p>Concrètement, toutes les communications entre ton PC, ton téléphone et ton Mac Mini passent par ce tunnel chiffré — complètement invisibles pour quiconque sur le réseau, y compris ton fournisseur d'accès internet, l'administrateur du réseau Wi-Fi, ou un pirate qui écoute le trafic.</p>
<p>Sans VPN, quand tu te connectes en SSH à ton Mac Mini depuis un café, un coworking, un hôtel, ou même depuis un réseau Wi-Fi domestique mal sécurisé, quelqu'un sur le même réseau pourrait théoriquement intercepter ta connexion avec des outils facilement trouvables. Avec un VPN, c'est <strong>mathématiquement impossible</strong> — le chiffrement utilisé (AES-256 ou ChaCha20) est le même que celui utilisé par les militaires et les banques.</p>
<p>Mais le VPN ne sert pas uniquement au chiffrement. Il résout aussi un problème très pratique : <strong>l'accès à distance</strong>. Ton Mac Mini est derrière ta box internet, qui lui attribue une adresse IP locale (type 192.168.x.x). Cette adresse n'est pas accessible depuis l'extérieur. Le VPN crée un réseau virtuel privé avec ses propres adresses IP, accessibles depuis n'importe où dans le monde. Tu peux te connecter à ton Mac Mini depuis le Japon, le Brésil ou le café d'en face — tant que tu es sur le VPN, c'est comme si tu étais branché en direct sur le même réseau local.</p>
<p><strong>Attention :</strong> Ne confonds pas un VPN "serveur" (comme Tailscale ou WireGuard self-hosted) avec un VPN "commercial" (comme NordVPN ou ExpressVPN). Les VPN commerciaux sont conçus pour masquer ton IP quand tu navigues sur internet. Ce n'est pas du tout ce dont on parle ici. Ce qu'on veut, c'est un VPN "mesh" qui connecte tes propres appareils entre eux de manière sécurisée.</p>
...
<ol>
<li><strong>Créer un compte Tailscale</strong> : Va sur tailscale.com et crée un compte. Tu peux utiliser ton compte Google, Microsoft, ou GitHub pour t'authentifier — ce qui active automatiquement la 2FA si ton compte source l'utilise. Choisis le plan "Personal" (gratuit) — il est largement suffisant pour commencer.</li>
<li><strong>Sur ton Mac Mini (le serveur)</strong> : Télécharge Tailscale depuis le Mac App Store ou installe-le via Homebrew avec <code>brew install tailscale</code>. Lance l'application et connecte-toi avec ton compte. Une adresse IP Tailscale (commençant par 100.x.x.x) est automatiquement attribuée à ton Mac Mini. Note cette adresse — c'est celle que tu utiliseras pour te connecter à distance.</li>
...
<li><strong>Sur ton téléphone</strong> : Installe l'app Tailscale depuis l'App Store (iOS) ou le Play Store (Android). Connecte-toi avec le même compte. Ton téléphone rejoint le réseau — tu pourras administrer ton serveur depuis ton lit ou en déplacement.</li>
<li><strong>Résultat immédiat</strong> : Tes 3 appareils (ou plus) sont maintenant sur un réseau privé chiffré. Tu peux accéder au Mac Mini via son IP privée Tailscale (ex: 100.64.x.x) depuis n'importe où dans le monde — un café à Paris, un aéroport au Japon, le réseau 4G de ton téléphone. La connexion est chiffrée de bout en bout avec WireGuard.</li>
</ol>
...
<p><strong>Astuce sécurité :</strong> Dans le panneau d'administration Tailscale (login.tailscale.com/admin), tu peux voir tous les appareils connectés, révoquer l'accès d'un appareil perdu ou volé, et configurer des ACLs (Access Control Lists) pour contrôler finement qui peut se connecter à quoi.</p>
...
<li><strong>Activer au démarrage</strong> : Configure WireGuard pour se lancer automatiquement au démarrage du système. Sur Linux : <code>sudo systemctl enable wg-quick@wg0</code>. Sur macOS, tu peux utiliser un Launch Daemon ou un script de démarrage. C'est essentiel pour un serveur : si la machine redémarre après une coupure de courant, le VPN doit revenir automatiquement.</li>
...
<li><strong>Test depuis un autre réseau</strong> : Si possible, teste depuis un réseau complètement différent — le réseau 4G de ton téléphone par exemple. Connecte-toi au VPN via l'app mobile, puis SSH dans le Mac Mini. Si ça marche, tu sais que ton setup fonctionne depuis n'importe où dans le monde.</li>
</ul>
<p><strong>Astuce de debugging :</strong> Si quelque chose ne marche pas, vérifie dans l'ordre : (1) Tailscale/WireGuard est-il actif sur les deux machines ? (2) Les deux machines se voient-elles dans le dashboard Tailscale ? (3) Le firewall local ne bloque-t-il pas les connexions ? La plupart des problèmes viennent du point 3.</p>
...
<li><strong>Activer l'authentification 2FA</strong> sur ton compte Tailscale (ou le service VPN que tu utilises). Si quelqu'un compromet ton mot de passe Tailscale, il pourrait ajouter ses propres appareils à ton réseau et accéder à ton serveur. Avec la 2FA, même avec ton mot de passe, il ne peut pas se connecter sans ton deuxième facteur d'authentification.</li>
<li><strong>Ne partager tes clés VPN avec personne</strong> : Chaque appareil doit avoir sa propre paire de clés. Ne copie jamais une clé privée d'un appareil à un autre. Si tu veux ajouter un nouvel appareil, génère une nouvelle paire de clés pour lui. Si un appareil est perdu ou volé, révoque immédiatement ses clés depuis le dashboard.</li>
<li><strong>Auditer régulièrement les appareils connectés</strong> : Va dans le dashboard Tailscale (ou ton interface WireGuard) et vérifie la liste des appareils. Tu reconnais tout ? Il n'y a pas d'appareil que tu n'utilises plus ? Retire les appareils obsolètes — un vieux laptop que tu as revendu et oublié de retirer du VPN, c'est une porte d'entrée potentielle.</li>
<li><strong>Garder le logiciel VPN à jour</strong> : Les mises à jour de Tailscale et WireGuard incluent régulièrement des correctifs de sécurité. Active les mises à jour automatiques ou vérifie manuellement chaque semaine.</li>
</ul>
...
<ol>
<li>Installe Tailscale sur ton Mac Mini (ou serveur) ET sur ton PC principal</li>
...
<p><strong>Livrable :</strong> Screenshot du dashboard Tailscale montrant tes appareils connectés.</p>`,
  },
  {
    ...
    slug: "firewall-ports-securite-reseau",
    duration: "55 min",
...
<p>Le firewall contrôle toutes les connexions réseau entrantes et sortantes de ta machine. Chaque fois qu'un appareil ou un programme essaie de se connecter à ton serveur, le firewall vérifie : "Est-ce que cette connexion est autorisée ?" Si oui, il la laisse passer. Si non, il la bloque silencieusement — l'attaquant ne sait même pas que ta machine existe.</p>
...
<ol>
<li><strong>Ouvrir les réglages</strong> : Va dans <strong>Réglages Système > Réseau > Coupe-feu</strong> (ou "Firewall" si ton macOS est en anglais). Sur les versions plus anciennes de macOS, c'est dans Préférences Système > Sécurité et confidentialité > Coupe-feu.</li>
...
<p>Quand quelqu'un essaie de se connecter à ton serveur, il frappe à une porte spécifique. Le firewall décide si cette porte est ouverte ou fermée, et pour qui.</p>
...
<li><strong>Port 443 (HTTPS)</strong> : Trafic web sécurisé (chiffré avec TLS). À ouvrir uniquement si ton agent expose une API ou une interface web accessible depuis l'extérieur. Si ton agent communique uniquement via des APIs tierces (OpenAI, Stripe, etc.), tu n'as pas besoin d'ouvrir ce port — ces communications sont des connexions sortantes, pas entrantes.</li>
...
<p>La sécurité réseau fonctionne en couches, comme un oignon (ou un château fort médiéval avec ses douves, ses remparts, ses tours, et son donjon). L'idée fondamentale est que <strong>même si une couche est compromise, les autres couches protègent encore</strong>. Un attaquant qui franchit une barrière se retrouve face à une autre. C'est ce qu'on appelle la "défense en profondeur".</p>
...
<ol>
<li>Active le firewall macOS (ou ufw sur Linux) avec le mode furtif</li>
...
<p><strong>Bonus :</strong> Installe Little Snitch (macOS) ou activez les logs pf pour monitorer le trafic.</p>
<p><strong>Livrable :</strong> Screenshot montrant le firewall actif + connexion SSH réussie via VPN uniquement.</p>`,
  },
  {
    ...
    slug: "reglages-securite-openclaw",
    duration: "50 min",
...
<p><strong>Astuce pro :</strong> Utilise un outil comme <code>dotenv-vault</code> ou <code>infisical</code> pour gérer tes secrets de manière encore plus sécurisée. Ces outils chiffrent tes .env et te permettent de les partager de manière sécurisée entre environnements sans jamais les exposer en clair. C'est un niveau au-dessus du simple fichier .env.</p>
...
<li><strong>Mettre à jour le .env sur le serveur</strong> avec la nouvelle clé. Si tu utilises Docker, reconstruis le container ou redémarre-le pour qu'il prenne en compte la nouvelle variable d'environnement.</li>
<li><strong>Vérifier que l'agent fonctionne</strong> avec la nouvelle clé. Fais un test rapide — envoie une requête à chaque API pour confirmer que tout marche. Ne passe pas à l'étape suivante tant que tu n'as pas confirmé.</li>
...
<ul>
<li><strong>Utilisateur système dédié</strong> : Ne fais pas tourner OpenClaw avec ton compte administrateur ou en tant que root. Crée un utilisateur système dédié (par exemple <code>openclaw</code>) avec des droits limités. Cet utilisateur n'a pas besoin de pouvoir installer des logiciels, modifier la configuration système, ou accéder aux fichiers des autres utilisateurs. Si l'agent est compromis, l'attaquant hérite des droits de cet utilisateur limité — pas de l'administrateur.</li>
...
<li><strong>API rate limiting</strong> : Configure des limites de taux (rate limits) sur les appels API pour éviter qu'un bug, une boucle infinie, ou une attaque ne fasse exploser ta facture. La plupart des APIs permettent de définir des alertes de budget. Sur OpenAI, par exemple, tu peux définir une limite de dépense mensuelle. Sur Stripe, tu peux configurer des webhooks qui t'alertent au-delà d'un certain montant de transactions. Configure ces limites AVANT qu'un incident se produise — après, il est trop tard.</li>
...
<p><strong>Attention :</strong> Les logs eux-mêmes peuvent contenir des informations sensibles (emails de clients, contenus de messages, etc.). Protège tes fichiers de log avec les mêmes précautions que ton .env : permissions restrictives, pas de commit dans Git, rotation régulière (les vieux logs sont compressés et archivés). Et ne logue jamais d'API keys, de mots de passe, ou de tokens dans les logs.</p>
...
<li><strong>Chiffrement en transit</strong> : Toutes les connexions API doivent être en HTTPS — jamais en HTTP. Vérifie que ton code utilise <code>https://</code> et pas <code>http://</code> pour tous les appels API. Les librairies modernes comme axios ou fetch refusent par défaut les connexions non sécurisées, mais vérifie quand même. Le VPN chiffre aussi le trafic entre tes appareils, mais le HTTPS ajoute une couche supplémentaire de chiffrement de bout en bout jusqu'au serveur de destination.</li>
<li><strong>Base de données</strong> : Si OpenClaw utilise une base de données locale (SQLite, PostgreSQL, etc.), active le chiffrement si disponible. Pour SQLite, tu peux utiliser SQLCipher qui ajoute le chiffrement AES-256 transparent. Pour PostgreSQL, active le chiffrement TLS pour les connexions et considère le chiffrement au niveau des colonnes pour les données les plus sensibles (comme les emails ou numéros de téléphone des clients).</li>
<li><strong>Backups chiffrés</strong> : N'oublie pas les sauvegardes. Un backup non chiffré sur un disque externe ou dans le cloud est une faille de sécurité. Utilise des outils comme <code>restic</code> ou <code>borgbackup</code> qui chiffrent automatiquement les sauvegardes. Si tu utilises un service cloud (Google Drive, iCloud, S3), active le chiffrement côté client en plus du chiffrement côté serveur.</li>
</ul>
<p><strong>Conseil :</strong> Le chiffrement a un coût en performance — mais avec les puces Apple Silicon, ce coût est négligeable. Le M2 et le M4 ont un moteur de chiffrement matériel dédié qui rend FileVault transparent en termes de performances. Tu ne sentiras aucune différence au quotidien.</p>`,
...
<li>Active FileVault (macOS) ou LUKS (Linux) pour le chiffrement du disque</li>
...
<p><strong>Livrable :</strong> Capture de ta configuration sécurisée (sans montrer les vraies API keys !).</p>`,
  },
  {
    ...
    slug: "maintenance-bonnes-pratiques-securite",
    duration: "50 min",
...
<p><strong>Astuce productivité :</strong> Automatise autant que possible. Un script qui vérifie l'uptime, l'espace disque et les dernières alertes, puis t'envoie un résumé sur Discord chaque matin, ça se met en place en 30 minutes et ça te fait gagner un temps considérable sur le long terme. Le but, c'est que ta routine quotidienne consiste à lire un message, pas à taper des commandes.</p>
...
<li><strong>Backup de la configuration</strong> : Sauvegarde ton fichier .env, tes configurations Docker (docker-compose.yml), ta configuration OpenClaw, et tes règles de firewall. <strong>Pas sur le même disque que le serveur !</strong> Utilise un disque externe chiffré, un service cloud chiffré (avec chiffrement côté client), ou un second serveur. La règle des sauvegardes est le 3-2-1 : 3 copies de tes données, sur 2 supports différents, dont 1 hors site. Ça peut sembler excessif, mais le jour où ton SSD lâche ou ton serveur est compromis, tu seras content d'avoir un backup fonctionnel.</li>
...
<li><strong>Revue de l'architecture</strong> : Ton setup est-il toujours adapté à tes besoins actuels ? Peut-être que tu as ajouté de nouveaux services, de nouveaux clients, ou de nouveaux agents depuis le dernier audit. Est-ce que les règles de firewall reflètent encore la réalité de ton infrastructure ? Faut-il ajouter des couches de sécurité supplémentaires ? Faut-il repenser certains choix ? C'est le moment de se poser ces questions stratégiques.</li>
<li><strong>Formation continue</strong> : La sécurité informatique évolue vite. Prends 20 minutes pour te mettre à jour : lis les dernières actualités sécurité pertinentes pour ta stack (Node.js, Docker, macOS). Vérifie les CVE (Common Vulnerabilities and Exposures) récentes qui pourraient affecter tes outils. Suis des comptes spécialisés sur Twitter/X ou des newsletters comme "tl;dr sec" ou "This Week in Security". Tu n'as pas besoin de devenir un expert en cybersécurité — mais tu dois rester informé des menaces qui te concernent directement.</li>
</ul>
...
<li><strong>Change TOUTES les API keys immédiatement</strong> : Depuis un autre appareil (ton PC ou ton téléphone), connecte-toi à chaque service et génère de nouvelles clés. OpenAI, Stripe, GitHub, Google — tout, sans exception. Révoque les anciennes clés immédiatement. Même si tu penses que seul un service a été compromis, change tout — tu ne sais pas ce que l'attaquant a pu voir ou copier. C'est la mesure la plus urgente après l'isolation.</li>
...
<li>Fais un backup complet de ta configuration actuelle (stocke-le sur un cloud chiffré ou un disque externe)</li>
